...
print_help() {
    cat <<EOF
Deye Solar Monitor - Release Cycle & Version Manager
...
echo -e "${CYAN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${CYAN}${BOLD}🚀 DEYE SOLAR MONITOR — RELEASE CYCLE ORCHESTRATOR${NC}"
echo -e "${CYAN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
if ! command -v gh > /dev/null 2>&1; then
    ...
    if [ "$NO_PUSH" = false ] && [ "$DRY_RUN" = false ]; then
        echo -e "${RED}Please install with 'brew install gh' or run with '--no-push'.${NC}"
        exit 1
    fi
fi
...
PBXPROJ_FILE="DeyeMacOS.xcodeproj/project.pbxproj"
CURRENT_VERSION=$(grep -m1 "MARKETING_VERSION" "$PBXPROJ_FILE" | sed -E 's/.*= ([^;]+);/\1/' | tr -d ' "')
CURRENT_BUILD=$(grep -m1 "CURRENT_PROJECT_VERSION" "$PBXPROJ_FILE" | sed -E 's/.*= ([^;]+);/\1/' | tr -d ' "')
...
if [ "$DRY_RUN" = false ]; then
    ...
    echo -e "${GREEN}✅ $PBXPROJ_FILE updated (MARKETING_VERSION: $NEW_VERSION, CURRENT_PROJECT_VERSION: $NEW_BUILD).${NC}"
else
    echo -e "${YELLOW}ℹ️  DRY-RUN: $PBXPROJ_FILE update skipped.${NC}"
fi
...
DIST_ZIP="$PROJECT_ROOT/dist/Deye-Solar-Monitor-v${NEW_VERSION}-macOS.zip"
DIST_SHA="$PROJECT_ROOT/dist/Deye-Solar-Monitor-v${NEW_VERSION}-macOS.zip.sha256"
...
if [ "$DRY_RUN" = false ]; then
    git add "$PBXPROJ_FILE" "$CHANGELOG_FILE"
...
if [ "$NO_PUSH" = false ] && [ "$DRY_RUN" = false ]; then
    ...
    if command -v gh > /dev/null 2>&1; then
        ...
        if gh release view "$TAG_NAME" > /dev/null 2>&1; then
            ...
            gh release edit "$TAG_NAME" --title "Deye Solar Monitor ${TAG_NAME}" --notes-file "$RELEASE_NOTES_FILE"
            if [ ${#ASSETS[@]} -gt 0 ]; then
            ...
            gh release create "$TAG_NAME" \
                "${ASSETS[@]}" \
                --title "Deye Solar Monitor ${TAG_NAME}" \
    ...
    echo -e "${YELLOW}ℹ️  GitHub release skipped due to --no-push or --dry-run.${NC}"
    echo -e "   To push manually:"
    echo -e "   git push origin $CURRENT_BRANCH && git push origin $TAG_NAME"
    if [ -f "$DIST_ZIP" ]; then
        echo -e "   gh release create $TAG_NAME \"$DIST_ZIP\" \"$DIST_SHA\" --title \"Deye Solar Monitor $TAG_NAME\" --notes-file \"$RELEASE_NOTES_FILE\""
    fi
fi
